...
object KMeansWordsTestMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val uberLoadFile =  sc.textFile("hdfs://quickstart.cloudera:8020/user/cloudera/textdoc")
    val uberHead = uberLoadFile.first()                 
                      
    val uberLoadSplitted = uberLoadFile.filter { x => !x.equalsIgnoreCase(uberHead) }
    .map { x => x.split(",") 
      }
    val uberLoadmap = uberLoadSplitted.map{ x => 
        Row(convertToTimeStamp(x(0).replaceAll("\"", "")),x(1).replaceAll("\"", "").toString())}
